...
```{r}
data_wide <- data %>%
  group_by(Song_ID, Songname, release_date, Song_Pop.2019, Song_Pop.2022, Song_Pop.2023a, Song_Pop.2023b) %>%
  mutate(artist_number = paste0("Artist_", row_number())) %>%
  pivot_wider(
    id_cols = c(Song_ID, Songname, release_date, Song_Pop.2019, Song_Pop.2022, Song_Pop.2023a, Song_Pop.2023b),
...
## Subset to Colaborations with Only 2 Artists
...
```{r}
(artist <- rownames(binded)[50])
data_sample %>%
  filter(Artist_1 == artist | Artist_2 == artist)
```
